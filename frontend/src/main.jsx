...
// Helmet shim — renders children as-is (no react-helmet CDN needed)
const Helmet = (props) => props.children || null;
...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.error) {
      return (
        <div style={{ padding: '2rem', fontFamily: 'monospace', background: '#fff3cd', border: '2px solid #e67e22', borderRadius: '8px', margin: '2rem' }}>
          <h2 style={{ color: '#c0392b' }}>⚠️ App Error (check browser console for details)</h2>
          <pre style={{ whiteSpace: 'pre-wrap', color: '#333', fontSize: '0.9rem' }}>{String(this.state.error)}</pre>
        </div>
      );
    }
...
function Navbar({ current }) {
  ...
  let user = null;
  try { const raw = localStorage.getItem("user"); if (raw) user = JSON.parse(raw); } catch(e) { localStorage.removeItem("user"); }
    ...
    <>
      ...
      <section className="sk-section sk-featured" ref={featuredRef}>
        ...
        {hasSearched && (selectedCity || selectedEventType) && (
          <p style={{margin:"-8px 0 16px",fontSize:16,color:"#795548",fontFamily:"'Caveat',cursive"}}>
